...
def generate_date_code(
    date, year, rows, old=1
):  # creates alternating colors for the date and public works
    ...
    if old:
        html = (
            ...
            + pink
            + "'>Public Works</span> event featured "
            + str(n_talks)
            + " talks:\n\n<br><br>"
        )
    else:
        html = (
            ...
            + "</span> <span style='color: "
            + pink
            + "'>Public Works</span> event will feature "
            + str(n_talks)
            + " talks:\n\n<br><br>"
        )
    return html
...
def generate_event(rows, old=1):
    ...
    for index, row in rows.iterrows():
        if (
            count % 2
        ):  # alternate colors by making even lines cyan pink and odd lines pink cyan
            color = [cyan, pink]
        else:
            color = [pink, cyan]
    ...
    allcodes = (
        allcodes + "<br><br><br><br>"
    )  # add some space to delineate different nights
...
def make_pages():
    with (
        open("preamble.html", "r") as file
    ):  # preamble contains the stuff that is at the top of every page, like the menu header and the logo
        preamble = file.read()

    with open(
        "about.html", "r"
    ) as file:  # this contains the text used to describe the event
        about = file.read()

    with open(
        "footer.html", "r"
    ) as file:  # footer contains the listerv sign up, email addresses for contact, etc.
        footer = file.read()

    with open(
        "datetime.html", "r"
    ) as file:  # datetime contains the date, location, and time of the event
        datetime = file.read()

    with open(
        "crowd.html", "r"
    ) as file:  # crowd contains an image of the crowd at the first event
        crowd = file.read()
    ...
    for i in np.arange(n_events - 1, 0, -1):  # for every entry except the last one
        ...
        rows = events.loc[
            events["event"] == i
        ]  # get locations of each row that matches the current event number
        # print(rows)
        nightcode = generate_event(
            rows
        )  # generate the html code to describe that night of talks
    ...
    with (
        open("past_events.html", "w", encoding="utf-8") as outfile
    ):  # utf-8 required to get 'special' characters like mac's bullshit quotes or whatever.
        outfile.write(htmlcode)
    with (
        open("index.html", "w", encoding="utf-8") as outfile
    ):  # utf-8 required to get 'special' characters like mac's bullshit quotes or whatever.
